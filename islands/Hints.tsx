...
export default function Hints({ guesses, CONFIG }: HintsProps){
    ...
    return (
        <div class={tw`px-2 text-xs text-center items-center justify-center flex flex-col gap-2`}>   
            <h3 class={tw`m-0 p-0 `}>The number is <strong class={tw`bg-${COLORS_VALUES['green']} px-1 pb-1 rounded hover:bg-green-300`}> {getPrime(NUMBER) ? '' : '!'}Pair </strong></h3> 
            <h3 class={tw`m-0 p-0 `}> & <strong class={tw`bg-${COLORS_VALUES['green']} px-1 pb-1 rounded  hover:bg-green-300`}> {getPair(NUMBER) ? 'Prime' : 'Odd'} </strong> </h3> 
        </div>      
    )
}